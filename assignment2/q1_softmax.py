...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x_exp=tf.exp(tf.subtract(x,tf.reduce_max(x,1,keepdims=True)))
    sum=tf.reduce_sum(x_exp,1,keepdims=True)
...
def cross_entropy_loss(y, yhat):
    ...
    y_multiply=tf.multiply(tf.to_float(y),yhat_log)
    out=tf.negative(tf.reduce_sum(y_multiply))
    ### END YOUR CODE
