...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        validLen=min(len(sentence),max_length)
        newSentence=sentence[0:validLen]+[zero_vector]*(max_length-validLen)
        newLabels=labels[0:validLen]+[zero_label]*(max_length-validLen)
        mask=[True]*validLen+[False]*(max_length-validLen)
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embedded=tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings),self.input_placeholder)
        embeddings=tf.reshape(embedded,[-1,self.max_length,self.config.n_features*self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b2=tf.get_variable("b2",shape=[self.config.n_classes,],dtype=tf.float32,initializer=tf.constant_initializer(0))
