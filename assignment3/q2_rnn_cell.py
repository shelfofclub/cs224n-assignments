...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            wx=tf.get_variable("W_x",shape=[self.input_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=233))
            wh=tf.get_variable("W_h",shape=[self.state_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=344))
            b=tf.get_variable("b",shape=[self.state_size,],initializer=tf.contrib.layers.xavier_initializer(seed=455))
