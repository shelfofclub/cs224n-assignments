...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            wr=tf.get_variable("W_r",shape=[self.state_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=233))
            ur=tf.get_variable("U_r",shape=[self.input_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=666))
            br=tf.get_variable("b_r",shape=[self.state_size,],initializer=tf.constant_initializer(value=0))
            wz=tf.get_variable("W_z",shape=[self.state_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=69))
            uz=tf.get_variable("U_z",shape=[self.input_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=96))
            bz=tf.get_variable("b_z",shape=[self.state_size],initializer=tf.constant_initializer(value=0))
            wo=tf.get_variable("W_o",shape=[self.state_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=23))
            uo=tf.get_variable("U_o",shape=[self.input_size,self.state_size],initializer=tf.contrib.layers.xavier_initializer(seed=2333))
            bo=tf.get_variable("b_o",shape=[self.state_size,],initializer=tf.constant_initializer(value=0))

            z=tf.sigmoid(tf.matmul(inputs,uz)+tf.matmul(state,wz)+bz)
            r=tf.sigmoid(tf.matmul(inputs,ur)+tf.matmul(state,wr)+br)
            o=tf.tanh(tf.matmul(inputs,uo)+tf.matmul(r*state,wo)+bo)
