...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        senlen=len(sentence)
        sentenceExt=[start]*window_size+sentence+[end]*window_size
        for i in range(window_size,window_size+senlen):
            oneWindowFea=[]
            for j in range(i-window_size,i+window_size+1):
                oneWindowFea.extend(sentenceExt[j])
            windowed_data.append((oneWindowFea,labels[i-window_size]))
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        w=tf.get_variable(name="W",shape=[self.config.n_window_features*self.config.embed_size,self.config.hidden_size],initializer=tf.contrib.layers.xavier_initializer(seed=233))
        b1=tf.get_variable(name="b1",shape=[self.config.hidden_size,],initializer=tf.contrib.layers.xavier_initializer(seed=344))
        u=tf.get_variable(name="U",shape=[self.config.hidden_size,self.config.n_classes],initializer=tf.contrib.layers.xavier_initializer(seed=455))
        b2=tf.get_variable(name="b2",shape=[self.config.n_classes],initializer=tf.contrib.layers.xavier_initializer(seed=566))

        h=tf.nn.relu(tf.matmul(x,w)+b1)
        hDrop=tf.nn.dropout(h,keep_prob=dropout_rate)
    ...
    def add_loss_op(self, pred):
        ...
        losses=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=self.labels_placeholder)
    ...
    def add_training_op(self, loss):
        ...
        adamOpter=tf.train.AdamOptimizer(learning_rate=self.config.lr)
        train_op=adamOpter.minimize(loss)
        ### END YOUR CODE
        return train_op
